...
if __name__ == '__main__':
    ...
    prs.add_argument("-random_seed", dest="random_seed", default=42, required=False, help=".\n")
    ...
    args.attribute_seed_set = eval(args.attribute_seed_set)
    ...
    out_csv = 'lr{}_g{}_eps{}'.format(args.alpha, args.gamma, args.epsilon)
    ...
    for epi in range(args.start_run, args.runs):
        ...
        if args.explore_strategy == "e-greedy" or args.explore_strategy == "adaptive":
            ...
            exploration = ddqn_UCB(args.ucb_c)
        ...
        if args.fixed:
            ...
            while not done['__all__']:
                ...
                for ts in deep_q_agents.keys():
                   actions_,random_, predict_q, current_state = deep_q_agents[ts].act(train=True) # current_state: get at 0 seconds, is recorded before the action is taken, should get next state as the current state for 10s
                ...
                # first selected action is {'0': 0}
                # print("actions main:", actions)
                # breakpoint()
                ...
                # print("actions:",actions,"current time:",env.sim_step)
                # breakpoint()
                for agent_id in deep_q_agents.keys():
                    ...
                    deep_q_agents[agent_id].get_data(next_state=s[agent_id], selected_action=selected_action, reward=r[agent_id])
        ...
        for ts in env.ts_ids:
            # one traffic signal
            if args.reward == "lane_based_delay" or args.reward == "acc_pressure" or args.reward == "acc_pressure_no_abs" or args.reward == "discharge_rate" or args.state=="9ele_tslg" or args.state=="6ele_queue_tslg":
        ...
        if epi % 100 == 0:
            ...
            for agent_id in deep_q_agents.keys(): 
                with action_path.open( "w") as f:      
                    json.dump( deep_q_agents[agent_id].freq_by_state_by_action, f)
