...
def gen_rou_varying(epi):
    prob = []
    random.seed(42)
    path = str(Path("nets/single-intersection/two_way/single-intersection_varying{}.rou.xml".format(epi)).absolute())
...
guiShape="passenger"/>
        ...
        for rou in ["westbound","eastbound"]:
            print('    <flow id="{}_{}" route="{}" begin="0" end="3600" probability="{}" type="typeWE" \
                departSpeed="max" departPos="base" departLane="best"/>'.format(rou,0,rou,800/3600),file=routes)

        for rou in ["northbound","southbound"]:
            print('     <flow id="{}_{}" route="{}" begin="3600" end="7200" probability="{}" type="typeWE" \
                departSpeed="max" departPos="base" departLane="best"/>'.format(rou,1,rou,500/3600),file=routes)
        ...
        for rou in ["westbound","eastbound"]:
            print('    <flow id="{}_{}" route="{}" begin="7200" end="10800" probability="{}" type="typeWE" \
                departSpeed="max" departPos="base" departLane="best"/>'.format(rou,2,rou,1000/3600),file=routes)
...
def generate_routefile(p1,p2,epi,step,road_type):
    ...
    random.seed(epi)  # make tests reproducible
...
if __name__ == '__main__':
    ...
    prs.add_argument("-episode_time", dest="episode_time", type=int, default=6*1800, required=False, help="episode_time.\n")
    ...
    """path__ = "C:\\Users\\lvl5569\\Desktop\\newest_code\\outputs\\constant\\flow\\dql\\varying\\state_6ele_den_tslg_flow_500.0_900.0_e-greedy\\q_table\\"
    ...
    if args.test_flow:
        folder = str(Path('./outputs/constant/flow/dql_varying/retrain_state_{}_flow_{}_{}_{}_new_state{}_varying/'.format(args.state,args.flow_ns*3600,args.flow_ew*3600,args.explore_strategy,args.new_state)))
        file_name = os.path.join(folder, "logging")
        folders = os.path.join(folder, "q_table")
        #path for saving q table
        old_folder = str(Path('./outputs/constant/flow/dql/state_6ele_den_tslg_flow_500.0_900.0_e-greedy_new_state_e_lr/q_table'))
        
        newpath_1 = Path(os.path.join(old_folder, "q_table1"))
        newpath_2 = Path(os.path.join(old_folder, "q_table2"))
        action_path = Path(os.path.join(old_folder, "action_dict"))
    ...
    for epi in range(args.runs):
        ...
        # print('vehNr:', vehNr)
        gen_rou_varying(epi)
        route_file_name = str(Path('nets/single-intersection/'+args.road_type+'/single-intersection_varying{}.rou.xml'.format(epi)).absolute())
        ...
        if epi == 0: # episode
            #path 1 2, None
            path_1 = read_table(newpath_1)
            path_2 = read_table(newpath_2)
            # action_dict = {}
            action_dict = read_table(action_path)
            action_dict_1 = {}
            action_dict_2 = {} 
        else:
        ...
        """end of one epi"""
        if epi == 600:
            newpath_1 = Path(os.path.join(folders, "q_table1_epi600"))
            newpath_2 = Path(os.path.join(folders, "q_table2_epi600"))
            for agent_id in dql_agents.keys():       
