...
def generate_routefile(p1,p2,epi,step,road_type):
    ...
    if road_type == "two_way":
        path = Path("nets/single-intersection/two_way/single-intersection_varying{}.rou.xml".format(epi)).absolute()
...
if __name__ == '__main__':
    ...
    if args.test_flow:
        # folder = str(Path('./outputs/constant/flow/dql/state_{}_flow_{}_{}_{}_new_state{}'.format(args.state,args.flow_ns*3600,args.flow_ew*3600,args.explore_strategy,args.new_state)+args.paths))
        folder = str(Path('./outputs/constant/flow/dql_varying/retrain_state_6ele_den_tslg_flow_500.0_900.0_e-greedy_new_stateTrue_varying'))
